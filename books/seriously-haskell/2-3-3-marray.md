...
[`MArray`] は型が複雑で、エラーが出ると泥沼にはまります。長いエラー文が真の原因を表しているとは限らず、意外と単純な原因があるかもしれません。例として、 `ST` モナドに関連した代表的なエラーと、原因・対処方法を紹介します。

型で詰まった際には、 [haskell-jp slack](https://haskell.jp/signin-slack.html) で質問するのがおすすめです。以下で紹介する例も、過去に僕が haskell-jp で質問し、手厚く助けて頂けたためにコンパイルできた関数の類型です。
...
[`runSTUArray`] によって累積和の配列を生成します。ここで `ST` (state thread) モナドの理解としては、 `do` 以下で可変変数を扱うことができるイメージです:
...
これは派手なコンパイルエラーになります。 `newArray` は型クラス `MArray` の関数ですが、どの配列のインスタンスを使用するか推論できなくなるためです。
...
`MArray` に対して働きかける関数を作るのは簡単で、 [`writeArray :: (MArray a e m, Ix i) => a i e -> i -> e -> m () `][`writeArray`] と同じように型を書けば良いです。
...
分かりました。 [TypeFamilies] 拡張は [`Data.Vector.Unboxed`] の型クラス `Unbox` を実装するために有効化していました。有効化するとなぜエラーが出るのかまでは分かりませんが、上手く推論が働くように調整すれば良いようです。
