...
```rust
fn times<T>(n: usize, f: impl Fn(&T) -> T, x0: T) -> T {
    let mut x = x0;
    for i in 0..n {
        x = f(&x);
    }
...
ところで 2023 年の AtCoder では Emacs Lisp を使うことができます。なんてこったい。 ELisp も Haskell と同様に、手続き的な `return` が無い言語です。そのためしばしば `kill-emacs` でプロセスを終了する様子が見られます。不思議な人たちね。
